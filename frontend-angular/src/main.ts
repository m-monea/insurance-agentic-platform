...
const CUSTOMER_API = 'https://insurance-customer-service.onrender.com';
const CLAIM_API = 'https://insurance-claim-service.onrender.com';
const AI_API = 'https://insurance-ai-agent-service.onrender.com';
...
class AppComponent {
  ...
  eliminaCliente(id: number) {
    ...
    this.http.delete(CUSTOMER_API + '/api/clienti/' + id)
      .subscribe(() => this.caricaClienti());
  }

  eliminaTuttiClienti() {
    if (!confirm('Vuoi eliminare tutti i clienti?')) return;
    this.http.delete(CUSTOMER_API + '/api/clienti')
      .subscribe(() => this.clienti.set([]));
  }
  ...
  eliminaSinistro(id: number) {
    ...
    this.http.delete(CLAIM_API + '/api/sinistri/' + id)
      .subscribe(() => this.caricaSinistri());
  }

  eliminaTuttiSinistri() {
    if (!confirm('Vuoi eliminare tutti i sinistri?')) return;
    this.http.delete(CLAIM_API + '/api/sinistri')
      .subscribe(() => this.sinistri.set([]));
  }
  ...
  eliminaTuttoLoStorico() {
    ...
    this.http.delete(AI_API + '/api/agente/storico')
      .subscribe(() => this.storico.set([]));
  }
